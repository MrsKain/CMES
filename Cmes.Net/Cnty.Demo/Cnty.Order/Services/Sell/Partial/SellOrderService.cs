...
namespace Cnty.Order.Services
{
    public partial class SellOrderService
    {
        ...
        public WebResponseContent InsertDemo_1(SellOrder sellOrder)
        {
            SellOrderList sellOrderList = new SellOrderList();
            sellOrderList.Order_Id = sellOrder.Id;
            sellOrderList.MO = "22";
            sellOrderList.ProductName = "测试产品";
            sellOrderList.Qty = 50;
            sellOrderList.Weight = 11;
            return repository.DbContextBeginTransaction(() =>
           {
               repository.DbContext.Add<SellOrder>(sellOrder);
               repository.DbContext.Add<SellOrderList>(sellOrderList);
               repository.DelEntity(new SellOrder() { Id= sellOrderList.Order_Id});
               repository.SaveChanges();
              return new WebResponseContent().OK();
          });

        }
        public WebResponseContent DeleteDemo_1(Guid id)
        {          
            return repository.DbContextBeginTransaction(() =>
            {
                repository.DelEntity(new SellOrder(){Id=id});
                repository.SaveChanges();
                return new WebResponseContent().OK();
            });
        ...
        // Cnty.Core.Utilities.HttpContext.Current.RequestServices
        // Cnty.Core.Utilities.HttpContext.Current.RequestServices.GetService(typeof(ICacheService))

        //5、使用HttpContext：Cnty.Core.Utilities.HttpContext
        ...
        //8、查看权限验证规则 Cnty.Core->Filters文件夹下权限控制类

        //9、其他封装了大量的常用扩展方法Cnty.Core->Extensions文件夹下(如：字符串扩展、表达式扩展、实体验证(EntityProperties)扩展方法)
        ...
        public override PageGridData<SellOrder> GetPageData(PageDataOptions options)
        {
            ...
            */
            ...
            QueryRelativeExpression = (IQueryable<SellOrder> queryable) =>
            {
                ...
                queryable = queryable.Where(x => x.CreateID == UserContext.Current.UserId.ToString() || userQuery.Contains(int.Parse(x.CreateID)));
                return queryable;
            };
