...
using Cnty.AppManager.Repositories;
        ...
        public override WebResponseContent Update(SaveModel saveModel)
        {
            ...
            UpdateOnExecuting = (App_ReportPrice price, object obj, object obj2, List<object> list) =>
           {
            ...
            UpdateOnExecuted = (App_ReportPrice price, object obj, object obj2, List<object> list) =>
            {
                ...
                //id=0的默认为新增的数据
                List<App_News> addList = newsList.Where(x => x.Id == Guid.Empty).ToList();
                //设置默认创建人信息
                addList.ForEach(x => { x.SetCreateDefaultVal(); });

                //获取所有编辑行
                List<Guid> editIds = newsList.Select(s => s.Id).ToList();
                addList.ForEach(x => { x.SetModifyDefaultVal(); });
                //从数据库查询编辑的行是否存在，如果数据库不存在，执行修改操作会异常
                List<Guid> existsIds = App_NewsRepository.Instance.FindAsIQueryable(x => editIds.Contains(x.Id)).Select(s => s.Id).ToList();
