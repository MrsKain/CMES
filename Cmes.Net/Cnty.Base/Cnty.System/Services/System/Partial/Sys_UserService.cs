...
namespace Cnty.System.Services
{
    public partial class Sys_UserService
    {
        ...
        public async Task<WebResponseContent> Login(LoginInfo loginInfo, bool verificationCode = true)
        {
            ...
            try
            {
                var user = await repository.FindAsIQueryable(x => x.UserName == loginInfo.UserName)
                    .FirstOrDefaultAsync();
        ...
        public override PageGridData<Sys_User> GetPageData(PageDataOptions pageData)
        {
            ...
            {
                roleId = pageData.Value.ToString();
            }
            QueryRelativeExpression = (IQueryable<Sys_User> queryable) =>
            {
                if (string.IsNullOrEmpty(roleId))
                {
                    ...
                    roleId = UserContext.Current.RoleId.ToString();
                }

                //查看用户时，只能看下自己角色下的所有用户
                List<string> roleIds = Sys_RoleService
                   .Instance
                   .GetAllChildrenRoleId(new Guid(roleId));
                roleIds.Add(roleId);
                //判断查询的角色是否越权
                if (roleId != UserContext.Current.RoleId.ToString() && !roleIds.Contains(roleId))
                {
                    roleId = "22222222-2222-2222-2222-222222222222";
                }
                return queryable.Where(x => roleIds.Contains(x.Role_Id.ToString()));
            };
        ...
        public override WebResponseContent Add(SaveModel saveModel)
        {
            ...
            {
                string roleId = userModel?.MainData?["Role_Id"].ToString() ?? Guid.Empty.ToString();
                if (roleId != Guid.Empty.ToString())
                {
                    string roleName = GetChildrenName(roleId);
                    if ((!UserContext.Current.IsSuperAdmin && roleId == Guid.Empty.ToString()) || string.IsNullOrEmpty(roleName))
        ...
        public override WebResponseContent Del(object[] keys, bool delList = false)
        {
            ...
            {
                string[] userIds = ids.Select(x => x.ToString()).ToArray();
                //校验只能删除当前角色下能看到的用户
                var delUserIds = repository.Find(x => userIds.Contains(x.Id.ToString()), s => new { s.User_Id, s.Role_Id, s.UserTrueName });
                List<string> roleIds = Sys_RoleService
                   .Instance
                   .GetAllChildrenRoleId(UserContext.Current.RoleId);

                string[] userNames = delUserIds.Where(x => !roleIds.Contains(x.Role_Id.ToString()))
            ...
            return base.DelEntity(keys);
        }

        private string GetChildrenName(string roleId)
        {
            //只能修改当前角色能看到的用户
            string roleName = Sys_RoleService
                .Instance
                .GetAllChildren(UserContext.Current.UserInfo.Role_Id).Where(x => x.Id.ToString() == roleId)
        ...
        public override WebResponseContent Export(PageDataOptions pageData)
        {
            ...
            QueryRelativeExpression = (IQueryable<Sys_User> queryable) =>
            {
                ...
                List<string> roleIds = Sys_RoleService
                 .Instance
                 .GetAllChildrenRoleId(UserContext.Current.RoleId);
                return queryable.Where(x => roleIds.Contains(x.Role_Id.ToString()) || x.User_Id == UserContext.Current.UserId);
            };
