...
namespace Cnty.System.Services
{
    public partial class Sys_MenuService
    {
        ...
        public List<Sys_Menu> GetUserMenuList(Guid roleId)
        {
        ...
        public async Task<object> GetMenuActionList(Guid roleId)
        {
            ...
            if (UserContext.IsRoleIdSuperAdmin(roleId))
            {
                return await Task.Run(() => GetAllMenu().Where(k=>k.IsDelete==0).Select(x =>
                new
                {
                    id = x.Menu_Id,
                    name = x.MenuName,
        ...
        public async Task<WebResponseContent> Save(Sys_Menu menu)
        {
            ...
            try
            {
                ...
                if (menu.Menu_Id <= 0)
                {
                    menu.Menu_Id= repository.GetEntityListAll().OrderByDescending(k => k.Menu_Id).Select(k => k.Menu_Id).FirstOrDefault() + 1;
                ...
                else
                {
                    ...
                    repository.UpdateEntity(() => new Sys_Menu() { ParentId =menu.ParentId, MenuName = menu.MenuName, Url= menu.Url, 
                        Auth= menu.Auth, OrderNo= menu.OrderNo, Icon= menu.Icon, Enable= menu.Enable, TableName= menu.TableName}, k => k.Menu_Id == menu.Menu_Id);
                }
