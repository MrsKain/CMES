...
namespace Cnty.Core.Dapper
{
    public class SqlDapper : ISqlDapper
    {
        ...
        private readonly IDictionary<int, IList<Action<IDbTransaction>>> _executeSqls = new ConcurrentDictionary<int, IList<Action<IDbTransaction>>>();
        ...
        /// <param name="commandType"></param>
        /// <returns></returns>
        public T ExecuteScalar<T>(string sql, object param = null, int? commandTimeout = null, CommandType? commandType = null, bool beginTransaction = false)
        {
            //using (var _conn = GetConnection())
            //{
            //    return _conn.ExecuteScalar<T>("SELECT *FROM SellOrder", param, null, commandTimeout, commandType);
            //}
            return Execute<T>((conn, dbTransaction) =>
            {
                return conn.ExecuteScalar<T>(sql, param, dbTransaction, commandType: commandType ?? CommandType.Text, commandTimeout: commandTimeout);
            }, beginTransaction);
        }
        ...
        public IEnumerable<T> Query<T>(string sql, object param = null, bool buffered = true, int? commandTimeout = null, CommandType? commandType = null)
        {
            using (var _conn = GetConnection())
            {
                return _conn.Query<T>(sql, param, null, buffered, commandTimeout, commandType);
            }
        }
        ...
        {
            if (!_executeSqls.ContainsKey(Thread.CurrentThread.ManagedThreadId))
            {
                _executeSqls.Add(Thread.CurrentThread.ManagedThreadId, new List<Action<IDbTransaction>>());
            }

            _executeSqls[Thread.CurrentThread.ManagedThreadId].Add(o =>
            {
                try
                {
                    o.Connection.Execute(sql, param, o,
                    commandTimeout, commandType);
                }
                catch (Exception ex)//捕获异常，打印出现异常的sql语句，方便调试用
                {
                    throw new Exception($"{ex.Message}_Execute:{sql}");
                    throw ex;
                }
        ...
        public void BatchInsert(DataTable dt, string tableName, Dictionary<string, string> columnNames)
        {
            if (!_executeSqls.ContainsKey(Thread.CurrentThread.ManagedThreadId))
            {
                _executeSqls.Add(Thread.CurrentThread.ManagedThreadId, new List<Action<IDbTransaction>>());
            ...
            _executeSqls[Thread.CurrentThread.ManagedThreadId].Add(o =>
            {
                using (SqlBulkCopy sqlbulkcopy = new SqlBulkCopy((SqlConnection)o.Connection, SqlBulkCopyOptions.Default, (SqlTransaction)o))
                {
                    try
                    {
                        ...
                        {
                            sqlbulkcopy.ColumnMappings.Add(columnName.Key, columnName.Value);
                        }
        ...
        {
            if (_executeSqls.Count <= 0
                || !_executeSqls.ContainsKey(Thread.CurrentThread.ManagedThreadId)
                || _executeSqls[Thread.CurrentThread.ManagedThreadId].Count <= 0)
            {
                return;
            }
            ...
            using (var _conn = GetConnection())
            {
                using (var tran = _conn.BeginTransaction())
                {
                    try
                    {
                        foreach (var executeSql in _executeSqls[Thread.CurrentThread.ManagedThreadId])
                        //foreach (var executeSql in _executeSqls.Value)
                        {
                            executeSql(tran);
                        }
                        tran.Commit();
                    ...
                    {
                        //清空当前执行的记录
                        _executeSqls[Thread.CurrentThread.ManagedThreadId].Clear();
                        _executeSqls.Remove(Thread.CurrentThread.ManagedThreadId);
                    }
                }
            }
        }
