...
namespace Cnty.Core.Infrastructure
{
    public static class DictionaryHandler
    {
        ...
        public static string GetRolesSql()
        {
            ...
            {
                return "SELECT \"Id\" as key,\"RoleName\" as value from Sys_Role";
            }
            return $@"SELECT Id as 'key',RoleName as 'value' FROM Sys_Role 
                           WHERE Enable=1 ";
        }
        ...
        public static string GetRolesSql(string originalSql)
        {
            ...
            string sql = $@"SELECT Id as 'key',RoleName as 'value' FROM Sys_Role 
                           WHERE Enable=1  and Id in ({string.Join(',', roleIds.Select(k => "'" + k + "'").ToArray())})";
            return sql;
        }
    }
}
