...
{
    public enum DirAwayCore
    {
        Left, Right
    }
    internal static class FuncExtensionsEntityFramewarkCore
    {

        #region Where Expression

        internal static StringBuilder Where<T>(this RepositoryBase<T> dataSource, Expression<Func<T, bool>> func, ref List<SqlParameter> paramerList) where T : BaseEntity, new()
        {
        ...
        static StringBuilder BinarExpressionProvider<T>(this RepositoryBase<T> dataSource, Expression left, Expression right, ExpressionType type, ref int index, ref List<SqlParameter> paramerList) where T : BaseEntity, new()
        {
        ...
        static StringBuilder ExpressionRouter<T>(this RepositoryBase<T> dataSource, Expression exp, DirAway away, ref int index, ref List<SqlParameter> paramerList, bool isFunc = false) where T : BaseEntity, new()
        {
            var sb = new StringBuilder();
            ...
            else if (exp is NewArrayExpression)
            {
                ...
                if (isFunc)
                {
                    ...
                    if (tmpstr.Length > 0)
                    {
                        ...
                        paramerList.Add(new SqlParameter(result, tmpstr.ToString()));
                        index++;
                    }
                }
                else
                {
                    ...
                    foreach (var ex in ae.Expressions)
                    {
                        ...
                        paramerList.Add(new SqlParameter(result, value.ToString()));
                ...
                if (away == DirAway.Right)
                {
                    if (!(ce.Value is Array))
                    {
                        ...
                        if (ce.Value == null || ce.Value is DBNull)
                        {
                            ...
                            paramerList.Add(new SqlParameter(result, DBNull.Value));

                        }
                        else if (ce.Value is ValueType)
                        {
                            //paramerList.Add(result, ce.Value);
                            paramerList.Add(new SqlParameter(result, ce.Value));
                        }
        ...
        internal static StringBuilder Order<T>(this RepositoryBase<T> dataSource, Expression<Func<OrderExpression<T>, object>> func) where T : BaseEntity, new()
        {
            var result = new StringBuilder();
            if (func != null)
            {
        ...
        internal static StringBuilder OrderCore<T>(this RepositoryBase<T>  dataSource, Expression<Func<OrderExpression<T>, object>> func) where T : BaseEntity, new()
        {
            var result = new StringBuilder();
            if (func != null)
            {
        ...
        static StringBuilder ExpressionOrderRoute<T>(this RepositoryBase<T> dataSource, Expression exp) where T : BaseEntity, new()
        {
            var result = new StringBuilder();
        ...
        /// <returns></returns>
        internal static StringBuilder UpdateEntity<T>(this RepositoryBase<T> dataSource, Expression<Func<T>> func, ref List<SqlParameter> paramerList) where T : BaseEntity, new()
        {
            var index = 0;
        ...
        /// <param name="paramerList"></param>
        /// <returns></returns>
        static StringBuilder ExpressionUpdateRouter<T>(this RepositoryBase<T> dataSource, Expression exp, DirAway away, ref int index, ref List<SqlParameter> paramerList) where T : BaseEntity, new()
        {
            ...
            else if (exp is ConstantExpression)
            {
                ...
                if (ce.Value == null || ce.Value is DBNull)
                {
                    ...
                    paramerList.Add(new SqlParameter(paramName, null));
                }
                else if (ce.Value is ValueType)
                {
                    //paramerList.Add(paramName, ce.Value);
                    paramerList.Add(new SqlParameter(paramName, ce.Value));
                }
                else if (ce.Value is string || ce.Value is DateTime || ce.Value is char)
                {
                    //paramerList.Add(paramName, ce.Value);
