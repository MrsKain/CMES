...
using Cnty.Entity.DomainModels.Sys;
using Cnty.Entity.SystemModels;

namespace Cnty.Builder.Services
{
...
DISTINCT
	...
	SELECT CASE WHEN t.ColumnType IN ('DECIMAL','smallmoney','money') THEN 
        ...
        public string CreateServices(string tableName, string nameSpace, string foldername, bool webController, bool apiController)
        {
            ...
            frameworkFolder + string.Format("\\{0}\\{0}\\IRepositories\\{1}\\", nameSpace, foldername),
            ...
            FileHelper.WriteFile(
           frameworkFolder + string.Format("\\{0}\\{0}\\Repositories\\{1}\\", nameSpace, foldername)
                          , tableName + "Repository.cs", domainContent);
           

            string path = $"{frameworkFolder}\\{nameSpace}\\{nameSpace}\\IServices\\{ foldername}\\";
            ...
            path = $"{frameworkFolder}\\{nameSpace}\\{nameSpace}\\Services\\{ foldername}\\";
            ...
            if (webController)
            {
                path = $"{frameworkFolder}\\{nameSpace}\\{nameSpace}\\Controllers\\{ foldername}\\";
        ...
        private string GetSqlServerStructure(string tableName)
        {
            ...
            SELECT TableName,
              ...
              CASE   WHEN ColumnType IN('time', 'date', 'DATETIME', 'smallDATETIME') THEN 150
                     ...
                     WHEN ColumnName IN('Modifier', 'Creater') THEN 130
        ...
        private void SetMaxLength(List<Sys_TableColumn> columns)
        {
            columns.ForEach(x =>
            {
                ...
                else if (x.ColumnName == "Modifier" || x.ColumnName == "Creater")
                {
                    x.ColumnWidth = 130;
                }
        ...
        private int InitTable(int parentId, string tableName, string columnCNName, string nameSpace, string foldername, int tableId, bool isTreeLoad)
        {
            ...
            if (tableId > 0)
                ...
                tableId = repository.GetEntityListAll().OrderByDescending(k => k.Table_Id).Select(k => k.Table_Id).FirstOrDefault() + 1;
        ...
        private string CreateEntityModel(List<Sys_TableColumn> sysColumn, Sys_TableInfo tableInfo, List<TableColumnInfo> tableColumnInfoList, int createType)
        {
            ...
            string mapPath = ProjectPath.GetProjectDirectoryInfo()?.FullName; //new DirectoryInfo(("~/").MapPath()).Parent.FullName;
                                                                              //  string folderPath= string.Format("\\Cnty.Framework.Core.\\DomainModels\\{0}\\", foldername);
            ...
            FileHelper.WriteFile(
                mapPath +
                string.Format(
                "\\" +"Cnty.Base\\"+ modelNameSpace + "\\DomainModels\\{0}\\", folderName
                )
                , tableName + ".cs",
                domainContent);
            if (createType == 1)
